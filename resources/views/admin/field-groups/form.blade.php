...
@section('breadcrumb')
    ...
    <a href="{{ route('cms.admin.field-groups.index') }}" class="text-gray-500 hover:text-gray-700">Custom Fields</a>
    <span class="mx-2 text-gray-300">/</span>
...
<div class="max-w-4xl mx-auto">
    ...
    {{-- ── BASIC INFO ───────────────────────────────────────────────── --}}
    <form method="POST"
          action="{{ $group->exists ? route('cms.admin.field-groups.update', $group->id) : route('cms.admin.field-groups.store') }}"
        ...
        <div class="p-5 space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="2"
                          class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm
                                 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $group->description) }}</textarea>
        ...
        <div class="flex items-center justify-between gap-3 px-5 py-3 bg-gray-50 border-t border-gray-100">
            <a href="{{ route('cms.admin.field-groups.index') }}" class="text-sm text-gray-600 hover:text-gray-900">Back to list</a>
            <button type="submit"
    ...
    @if($group->exists)
        @include('cms::admin.field-groups.fields-builder', ['group' => $group])
    @else
